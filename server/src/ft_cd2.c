...
/*   Updated: 2015/10/28 16:06:53 by frcugy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				check_error(char *new_path, char *path, t_uenv *env)
{
	...
	{
		ft_putendl_fd(ft_strjoin("cd: ", ft_strjoin(path, " permission denied"))
			, env->cs);
		return (0);
	}
	if (chdir(new_path) != 0)
	{
		ft_putendl_fd(ft_strjoin("cd: no such file or directory: ", path),
		env->cs);
		return (0);
	}
	if (chdir(new_path) == 0 && strncmp(getcwd(ft_strnew(1024), 1024),
	env->home, ft_strlen(env->home)) != 0)
	{
